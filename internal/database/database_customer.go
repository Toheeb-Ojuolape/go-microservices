...
import (
	...
	"github.com/Toheeb-Ojuolape/go-microservices/internal/dberrors"
...
func (c Client) GetAllCustomers(ctx context.Context, emailAddress string) ([]models.Customer, error) {
	...
	result := c.DB.WithContext(ctx).Where(models.Customer{Email: emailAddress}).
		Find(&customers)
...
func (c Client) GetCustomerById(ctx context.Context, ID string) (*models.Customer, error) {
	...
	result := c.DB.WithContext(ctx).Where(&models.Customer{CustomerID: ID}).
		First(&customer)
